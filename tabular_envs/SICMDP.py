...
from shellingham.posix import QEMU_BIN_REGEX
from tensorflow.security.fuzzing.py.annotation_types import Float32
...
class SICMDPEnv(gym.Env):
    ...
    def check_pi(self, pi: Tensor):
        ...
        assert torch.allclose(torch.sum(pi, dim=1), torch.ones((self.S), device=self.device).to(torch.ones((self.S)).dtype))
        return
    ...
    def Obj_pi(self, pi: Tensor) -> float:
        ...
        V_pi = self.V_pi(pi).to(torch.float32)
    ...
    def d_pi(self, pi: Tensor) -> Tensor:
        ...
        P_pi = self.P_pi(pi).to(torch.float32)
    ...
    def sample_based_NPG(self, pi: Tensor, train_Q_traj_s: Tensor, train_Q_traj_a: Tensor, train_V_traj_s: Tensor,
        ...
        if gt_advantage_flag:
            ...
            if b.dim()==2:
                ...
                # ▽ln(pi)
                score_pi = batch_torch_derivative_log_softmax(prob=pi, s_array=train_init_s_array,
                                                              a_array=train_init_a_array,
                                                              device=self.device)
                ...
                for k in range(train_traj_num - 1):
                    ...
                    G = 2. * (1 - self.gamma) * (np.tensordot(score_pi[k + 1], w_array[k]) * (1 - self.gamma) -
                                                 A_b_hat[k + 1]) * score_pi[k + 1]  # 用采集的G回报作为v
                    w_array[k + 1] = np_project_to_l2_ball(w_array[k] - lr * G, W)
                w_array = torch.as_tensor(w_array, device=self.device)
                gamma_k_array = ((2. * torch.arange(train_traj_num, device=self.device) + 1.) / (
                            train_traj_num * (train_traj_num + 1))).reshape((-1, 1, 1))

                return torch.sum(gamma_k_array * w_array, dim=0)
            elif b.dim()==3:

                # gamma_array_expanded = gamma_array.unsqueeze(0).expand(train_traj_num, train_traj_len)
                c = []
                ...
                for i in range(b.shape[0]):
                    lr = inner_init_lr  # eta_0
                    Q_b_hat = torch.sum(b[i][train_Q_traj_s, train_Q_traj_a] * gamma_array, dim=1)  # (train_traj_num,)
                    V_b_hat = torch.sum(b[i][train_V_traj_s, train_V_traj_a] * gamma_array, dim=1)  # (train_traj_num,)
                    A_b_hat = Q_b_hat - V_b_hat
                    #c.append(A_b_hat)
                    # numpy is faster here.
                    A_b_hat = A_b_hat.cpu().numpy()
                    ...
                    gamma_k_array = ((2. * torch.arange(train_traj_num, device=self.device) + 1.) / (
                                train_traj_num * (train_traj_num + 1))).reshape((-1, 1, 1))
                    c.append(torch.sum(gamma_k_array * w_array, dim=0))

                c = torch.stack(c)   # (y_size,self.S,self.A)
                return c
                ...
                while True:
                    try:
                        ...
                        train_init_traj_num_idx_array = torch.tensor(
                            np.random.choice(a=traj_num, size=train_traj_num, replace=True), device=self.device)
                        train_init_traj_len_idx_array = torch.tensor(
                            np.random.choice(a=traj_len, size=train_traj_num, replace=True), device=self.device)
                        train_init_s_array = traj_s[
                            train_init_traj_num_idx_array, train_init_traj_len_idx_array]  # (train_traj_num,)
                        train_init_a_array = traj_a[
                            train_init_traj_num_idx_array, train_init_traj_len_idx_array]  # (train_traj_num,)
                        del traj_s, traj_a
                        if self.empty_cache_flag:
                            torch.cuda.empty_cache()
                        # 按照1000初始状态及动作，采样Q的轨迹
                        train_Q_traj_s, train_Q_traj_a = self.sample_trajectories(pi=pi, traj_num=train_traj_num,
                                                                                  traj_len=train_traj_len,
                                                                                  init_flag=True,
                        ...
                        V_train_init_a_array = torch_vectorize_choice(p_tensor=pi[train_init_s_array, :],
                                                                      device=self.device)  # (train_traj_num,)
                        ...
                        # 使得回报最大的 Vr的梯度 (8,4)
                        V_r_hat_gradient = self.sample_based_NPG(pi=pi, train_Q_traj_s=train_Q_traj_s, train_Q_traj_a=train_Q_traj_a,
                        ...
                        V_c_hat_gradient = self.sample_based_NPG(pi=pi, train_Q_traj_s=train_Q_traj_s,
                                                                 train_Q_traj_a=train_Q_traj_a,
                                                                 train_V_traj_s=train_V_traj_s,
                                                                 train_V_traj_a=train_V_traj_a,
                                                                 W=W, inner_init_lr=inner_init_lr,
